...
export default async function handler(req,res){
 ...
 try{
  ...
  const pdfOutputRule=title==='Gerador de PDF'?'\n\nRetorne somente o texto final do material, sem JSON, sem código Markdown e sem escrever o nome da ferramenta no início. Estruture o material com: introdução objetiva, objetivos, seções numeradas, passos práticos, exemplos aplicáveis, checklist final e conclusão com próximos passos. Use títulos em linhas separadas, parágrafos curtos e listas simples. Não repita o título nem os dados do formulário como texto solto.':'';
  ...
  async function callGroq(){
   ...
   return {answer:clean(body?.choices?.[0]?.message?.content,10000),model:GROQ_MODEL,provider:'groq'};
  }
  async function callGemini(){
   ...
   const response=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(GEMINI_MODEL)}:generateContent?key=${encodeURIComponent(process.env.GEMINI_API_KEY)}`,{method:'POST',signal:AbortSignal.timeout(25000),headers:{'Content-Type':'application/json'},body:JSON.stringify({systemInstruction:{parts:[{text:systemMessage}]},contents,generationConfig:{temperature:isChat?0.35:0.45,maxOutputTokens:1000}})});
   const body=await response.json().catch(()=>({}));
   if(!response.ok){const error=new Error(body?.error?.message||'Gemini indisponível');error.status=response.status;throw error;}
   return {answer:clean(body?.candidates?.[0]?.content?.parts?.map(part=>part?.text||'').join(' '),10000),model:GEMINI_MODEL,provider:'gemini'};
  }
  let result=null,lastError=null;
  if(process.env.GROQ_API_KEY){try{result=await callGroq();}catch(error){lastError=error;console.error('GROQ',error.status||'',error.message||'');}}
  if(!result?.answer&&process.env.GEMINI_API_KEY){try{result=await callGemini();}catch(error){lastError=error;console.error('GEMINI',error.status||'',error.message||'');}}

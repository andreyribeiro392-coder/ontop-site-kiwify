(function(){
  ...
  function mount(attempt){
    ...
    async function call(action,payload){
      const response=await fetch('/api/email-auth',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,...payload})});
    ...
    requestForm.onsubmit=async event=>{
      ...
      try{await call('request',{email});requestForm.hidden=true;verifyForm.hidden=false;back.style.display='block';copy.textContent='Enviamos um código de seis números para '+email+'.';message.textContent='Confira sua caixa de entrada e a pasta de spam.';codeInput.focus();}
    ...
    verifyForm.onsubmit=async event=>{
      ...
      try{
        const body=await call('verify',{email,code});

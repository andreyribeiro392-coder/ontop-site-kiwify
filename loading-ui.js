...
(function(){
  ...
  function syncPreviewLabels(){
    ...
    const quota=document.querySelector('#ai-remaining');if(quota&&quota.textContent!=='5')quota.textContent='5';const metric=document.querySelector('.command-strip>div:nth-child(4)');if(metric){const value=metric.querySelector('strong'),label=metric.querySelector('span');if(value&&value.textContent!=='5')value.textContent='5';if(label&&label.textContent!=='respostas na prévia')label.textContent='respostas na prévia';}

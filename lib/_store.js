...
export async function createAccess({email='',name='',orderId='',source='admin',expiresAt=null}){let code=makeCode();while(await getJson(`access:${code}`))code=makeCode();const normalizedEmail=String(email).toLowerCase().trim();const access={code,email:normalizedEmail,name,orderId,source,status:'active',createdAt:new Date().toISOString(),activatedAt:null,lastAccessAt:null,expiresAt,devices:[],maxDevices:1};await setJson(`access:${code}`,access);await sadd('access:index',code);if(normalizedEmail)await setJson(`email:index:${tokenHash(normalizedEmail)}`,{code});if(orderId)await setJson(`order:${orderId}`,{code});return access;}
